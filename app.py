...
with tabs[8]:
    ...
    st.markdown(
        ...
        "on the fit parameters, the yield curve, the water trend and the cap "
        "|\n"
        ...
        "the fluid sets**, and against water cut with depletion partialled "
        "out |\n")
    ...
    st.markdown(
        ...
        "without it.\n\n"
        "That second test is **confounded on most wells**, and the app now "
        "says so rather than reporting the raw correlation. Water cut and "
        "cumulative production almost always rise together, so a CGR that "
        "climbs with depletion will correlate with water cut whether or not "
        "water has anything to do with it. The partial correlation — CGR "
        "against water cut holding cumulative gas fixed — is reported "
        "alongside, and when it collapses while the depletion term survives, "
        "the water explanation is withdrawn and a metering, allocation or "
        "emulsion cause is named instead. The ceiling test is the robust one; "
        "the water-cut regression is corroboration at best.")
    ...
    st.markdown(
        ...
        "Average reservoir pressure comes from a **measured** survey on the "
        "same row wherever one exists, and only falls back to the material "
        "balance when none does. That matters on a supported reservoir: the "
        "volumetric p/z relation assumes no influx, so it drives p_avg below "
        "the measured flowing pressure and the diagnostic refuses to run on a "
        "well that has perfectly good drawdown pairs. The source is named in "
        "the output. Flowing pressures are read from the full record rather "
        "than the rate-filtered one, since a gauge reading is a measurement "
        "of the well and has nothing to do with whether that month's rate was "
        "usable.\n\n"
    ...
    st.markdown(
        ...
        "AICc, the small-sample correction, because a post-plateau window "
        "often leaves fewer than twenty points and plain AIC under-penalises "
        "parameters there. Every candidate's pinned parameters are listed in "
        "the ranking table, not just the selected one's, and the raw rate "
        "trend is reported over several fitting windows so you can see how "
        "much of the answer is the window.\n"
    ...
    st.markdown(
        ...
        "single survey. An estimate is then labelled as *estimated, not "
        "measured* everywhere it is used — and because it is derived from the "
        "early p/z trend, fitting the p/z line back through it is circular. "
        "The material balance reports what the intercept becomes with that "
        "point dropped, so the leverage it carries is visible rather than "
        "buried in a tightened standard error.\n"
        ...
        "- **An aquifer fit that did not converge.** Checked on rms, on "
        "influx as a fraction of pore volume, on whether the best-fit G falls "
        "inside its own locus, and on the aquifer's own size — a Wei hundreds "
        "of times the hydrocarbon pore volume is the optimiser declaring the "
        "aquifer infinite, not a measurement. A fit that fails any of these is "
        "excluded from the gas-in-place choice and marked as rejected beside "
        "its own number.\n"
        ...
        "- **A wellstream that is more than 100 % condensate.** The "
        "wellstream *is* separator gas plus the gas equivalent of the "
        "condensate, so the yield has a hard ceiling at `1e6 / v_eq` "
        "STB/MMscf. Above it the forecast would produce more liquid than "
        "there is stream to carry it.\n"
        "- **A repeated date.** Two rows on one date make the period between "
        "them zero, count the volume twice and stop time increasing. If the "
        "repeats carry different rates it usually means two wells have been "
        "merged into one series, and it says so.\n"
        "- **A decline rate that is not distinguishable from zero.** When the "
        "standard error on D_i exceeds D_i itself, the well shows no "
        "measurable decline and any forecast decline is coming from the "
        "terminal D_min you assumed.\n"
        "- **A rate that is rising.** Every model here is bounded to a "
        "non-negative decline, so a building-up well cannot be represented — "
        "the near-zero D_i and low R\u00b2 are that bound being hit. The raw "
        "trend is reported so the cause can be found instead of fitted.\n"
    ...
    st.markdown(
        ...
        "**The Monte Carlo samples the fitted parameters, the yield curve, "
        "the water trend and the cap — but not the choice of model.** Two "
        "models can fit the same history equally well and forecast very "
        "differently, and that spread does not appear in P90/P10. Nor does "
        "the fitting window, which is usually the larger lever of the two.\n"
        "> *When it bites:* when the model ranking on the Decline fit tab "
        "shows several models within a point or two of AICc, or when the "
        "window check shows the trend moving between windows. Compare their "
        "EURs directly instead of reading the band.\n\n"
        "**Where the fitting window starts can decide the forecast.** The "
        "plateau detector is a heuristic, and on a well that never clearly "
        "leaves plateau it will cut somewhere defensible but arbitrary. Every "
        "number downstream — the decline, the EUR, the reserves — follows "
        "that cut.\n"
        "> *When it bites:* the window check under the selected fit reports "
        "the same trend over the full record, the fitted window and the last "
        "half. If they disagree in sign, or if the fitted window is the only "
        "one without a significant trend, the window is deciding the answer "
        "and it says so.\n\n"
        "**Most of an EUR can be extrapolation.** On a short or "
        "plateau-dominated history the forecast can contribute several times "
        "the produced volume, and a fit that is not identifiable will still "
        "return a number.\n"
        "> *When it bites:* the forecast reports the share of the EUR that is "
        "forecast rather than history whenever that share exceeds half and "
        "the fit is pinned or has a low R\u00b2. Read the share before the "
        "total.\n\n"
